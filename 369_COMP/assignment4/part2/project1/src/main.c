...
void setup_screen() {	
	...
	BITMAP *temp = (BITMAP*)data[THE_IRIS_NEBULA_BMP].dat;
...
void initialize_datafile() {
	data = load_datafile("./assets/spacetime.dat");
}
...
void setup_sounds() {
	...
	music = (SAMPLE*)data[BACKMUSIC_WAV].dat;
	if(!music) {
...
void* adjustEnemies_thread(void* _startIndex){

	int startIndex = (int)_startIndex;

	while(!gameover) {
			clear_enemies(startIndex);
			update_enemies(startIndex);
			draw_enemies(startIndex);
...
void* adjustPlayer_thread(void* tmp) {
	while(!gameover) {
			clear_ship();
			if(keypressed()) {
				getInput();
...
void setup_ship() {
	...
	ship_bmp = (BITMAP*)data[SPEEDSHIP_BMP].dat;
	if(ship_bmp == NULL) {
		printf("Error loading player ship bitmap.\n");
	}
...
void setup_enemies() {
	...
	enemy_bmp[0] = (BITMAP*)data[ENEMY_VERT_BMP].dat;
	enemy_bmp[1] = (BITMAP*)data[ENEMY_HORZ_BMP].dat;
...
void initialize_barriers() {
    int rc = pthread_barrier_init(&barr, NULL, NUM_ENEMIES / ENEM_PER_THREAD + 2);
    if(rc) {
...
void initialize_threads() {
	int i;
	for(i = 0; i < NUM_ENEMIES / ENEM_PER_THREAD; ++i) {
		if(pthread_create(&enem_threads[i], NULL, *adjustEnemies_thread, (void*)(i * ENEM_PER_THREAD))) {
			printf("\n thread init failed\n");
		}
	}

	pthread_create(&player_thread, NULL, *adjustPlayer_thread, NULL);
	pthread_create(&ui_thread, NULL, *drawUI_thread, NULL);
}
...
void clear_enemies(int startIndex) {
	int i;
	for(i = startIndex; i < startIndex + ENEM_PER_THREAD; i++){
...
void update_enemies(int startIndex) {
	int i;
...
void draw_enemies(int startIndex) {
	int i;
	for(i = startIndex; i < startIndex + ENEM_PER_THREAD; i++) {
		int x = enemies[i].x;
		int y = enemies[i].y;
