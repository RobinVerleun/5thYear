...
{
	data = load_datafile("./assets/flappy.dat");
	if(data == NULL) {
		printf("Error laoding datafile.\n");
		exit(1);
	}
}
...
void initialize_music() 
{
	...
	//load the wav files
	//music = load_sample("./assets/temp/hundouluo.wav");
	music = (SAMPLE*)data[HUNDOULUO_WAV].dat;
	//music = (SAMPLE*)data[HUNDOULUO_WAV].dat;
	if(!music) {
...
void load_playerImage() 
{
	BITMAP *temp;
	int n;
	temp = (BITMAP*)data[SPACESHIP_BMP].dat;
	for(n = 0; n < 3; n++) {
